...
class Auth extends CI_Controller
{
    ...
    //function untuk mengirim veifikasi 
    private function _sendEmail($token, $type)
    {
        ...
        $config['smtp_user'] = 'yunitarpl3@gmail.com';
        $config['smtp_pass'] = 'mniymvvjluaojang';
        $config['smtp_port'] = 465;
        $config['mailtype'] = 'html';
        $config['charset'] = 'utf-8';
        $this->email->initialize($config);
        $this->email->set_newline("\r\n");
        $this->email->from('yunitarpl3@gmail.com', 'yunita rpl');
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60*60*24)) {
                    ...
                    Account activication failed|token expired.</div>');
                    redirect('auth');
                }
            } else {
                $this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
                Account activication failed|Wrong Email</div>');
                redirect('auth');
            }
            } else {
                $this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
